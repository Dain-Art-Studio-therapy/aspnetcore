...
namespace acquire
{
    class Program
    {
        ...
        static string GetDotnetPath()
        {
            ...
            if (mainModule is not null && Path.GetFileName(mainModule) == fileName)
            {
                return mainModule;
            }
        ...
        static int Main(string[] args)
        {
            ...
            try
            {
                ...
                var targetManifestDirectory = Path.Combine(sdkDirectory, "sdk-manifests", "Microsoft.NET.Sdk.BlazorWebAssembly.AOT", ManifestVersion);
                ...
                File.WriteAllBytes(Path.Combine(sdkDirectory, "sdk", sdkVersion, "EnableWorkloadResolver.sentinel"), Array.Empty<byte>());
            }
            finally
            {
